            ...
            <div key={p.id} className="group">
              <Link to="/products/$id" params={{ id: p.id }}>
                <div className="relative aspect-[4/5] overflow-hidden rounded-xl bg-card">
                  <img src={p.image} alt={p.name} loading="lazy" width={900} height={1100} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
                  {p.badge && (
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">{p.moq} · {p.pack}</div>
                </div>
              </Link>
              <a
                href={waLink(p.name)}
